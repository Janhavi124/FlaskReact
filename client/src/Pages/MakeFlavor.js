...
function MakeFlavor() {
  ...
  useEffect(() => {
    fetch("/api/https://flaskreact-production-6221.up.railway.app/flavors")
      .then((res) => res.json())
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const res = await fetch("/api/https://flaskreact-production-6221.up.railway.app/calculate_flavor", {
      method: "POST",
  ...
  const res = await fetch("/api/https://flaskreact-production-6221.up.railway.app/save_batch", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ flavorname: selectedFlavor, bottles }),
  });
